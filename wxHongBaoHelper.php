...
class WxHongBaoHelper
{
	...
	function __construct($parinerkey)
	{
        $this->parinerkey = $parinerkey;
	}
	...
	protected function get_sign(){
		...
		try {
			if (null == $this->parinerkey || "" == $this->parinerkey ) {
			...
			return $md5SignUtil->sign($unSignParaString,$commonUtil->trimString($this->parinerkey));
	...
	*/
	...
	function curl_post_ssl($url, $vars, $second=30,$aHeader=array())
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT,dirname(__FILE__).DIRECTORY_SEPARATOR.'zs'.DIRECTORY_SEPARATOR.'apiclient_cert.pem');
		curl_setopt($ch,CURLOPT_SSLKEY,dirname(__FILE__).DIRECTORY_SEPARATOR.'zs'.DIRECTORY_SEPARATOR.'apiclient_key.pem');
		curl_setopt($ch,CURLOPT_CAINFO,dirname(__FILE__).DIRECTORY_SEPARATOR.'zs'.DIRECTORY_SEPARATOR.'rootca.pem');
